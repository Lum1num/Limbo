...
function Window:AddLeftGroupboxWithDescription(tab, title, desc)
    ...
    if desc ~= "" then
        formattedTitle = title .. ' <font size="10" color="rgb(80,80,80)">(' .. desc .. ')</font>'
    end
    return tab:AddLeftGroupbox(formattedTitle)
end
function Window:AddRightGroupboxWithDescription(tab, title, desc)
    ...
    return tab:AddRightGroupbox(formattedTitle)
end
...
tools1:AddButton({
    Text = "Serverhop",
    Func = function()
        cloneref(game:GetService("TeleportService")):TeleportToPlaceInstance(game.PlaceId, (function(s) return s.data[math.random(1,#s.data)].id end)(cloneref(game:GetService("HttpService")):JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100"))), cloneref(game:GetService("Players")).LocalPlayer)
    end,
